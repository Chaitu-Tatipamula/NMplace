...
const NFTCardOneInfo = ({ className = "",key,token, image, owner, NFT, isListed, onClick,  handleOpenSellModal, handleOpenTransferModal, removeNftListing, handleClose }) => {
    ...
    <div onClick={onClick} className={`relative cursor-pointer bg-chocolate-400  shadow-lg rounded-lg overflow-hidden ${className}`} style={{height:'400px', width: '100%' }} >
      ...
      <div className="absolute bottom-0 left-0 w-full backdrop-filter backdrop-blur-lg rounded-lg bg-gray-300 box-border flex md:flex-row items-start md:items-center justify-between p-4 gap-4 z-10 border border-solid border-gray-500">
        ...
        {owner !== signedAccountId && (
          ...
          <button onClick={(e) => { e.stopPropagation(); toggleMenu(key) }} className="cursor-pointer py-2 px-6 bg-transparent rounded-full bg-gradient-to-r from-blue-200 to-orange-400 flex items-center justify-center z-20 border border-solid border-tomato-100 hover:bg-orange-500">
            <div className="relative text-lg font-medium text-white">
              Options
            </div>
              {menuOpen === key && (
                <div className="absolute mt-4 bg-chocolate-700 w-40 shadow-lg z-10 rounded">
                  {
                    isListed == "Not Listed" ?

                    <div className="py-2 text-white">
                      {owner === signedAccountId && (
                        <button onClick={() => handleOpenSellModal( token.token_id)} className="flex items-center justify-start gap-2 w-full px-2 py-2 hover:bg-chocolate-500 transition-colors duration-300"><MdOutlineBackspace /> Sell</button>
                      )}
                       {owner === signedAccountId && (
                        <button onClick={() => handleOpenTransferModal(`${token.token_id}`)} className="flex items-center justify-start gap-2 w-full px-2 py-2 hover:bg-chocolate-500 transition-colors duration-300 rounded-b"><BiTransfer /> Transfer</button>
                      )}
                    </div>
